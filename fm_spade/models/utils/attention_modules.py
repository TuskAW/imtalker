...
class UnifiedSwinBlock(nn.Module):
    ...
    def forward(self, query, key=None, value=None):
        ...
        x = x + self.mlp(self.norm_ffn(x))
        return x.transpose(1, 2).view(B, C, H, W)
class ThreeCrossAttentionLayerFactory(nn.Module):
    """两步交叉注意力工厂：
       Step1: A-B 对齐 -> D
       Step2: D-C 结合 -> 最终输出
    """
    def __init__(self, args, dim, resolution):
        ...
        if self.is_standard_attention:
            print(f"  -> [工厂] 创建 STANDARD 双阶段交叉注意力层 @ {resolution}")
            # 第一步：A-B
            self.block_efc = StandardUnifiedAttention(dim=dim, num_heads=args.num_heads)
        else:
            print(f"  -> [工厂] 创建 GUIDED SPARCE 双阶段交叉注意力层 @ {resolution}")
            ...
            # 细粒度模式只对第二步做采样
            #self.block_ab = GuidedResampler(dim=dim, downsample_ratio=int(ratio))
            self.block = GuidedResampler(dim=dim, downsample_ratio=int(ratio))
    
    def coarse_warp(self, A, B, C, attn=None):
        B_, C_, H, W = A.shape
    ...
    def fine_warp(self, C, attn=None):
    ...
    def forward(self, A, B, C, D, attn=None):
        ...
        if not self.is_standard_attention:
            # --- Step1: A-B 对齐 ---
            #D = self.block_ab(B, attn.mean(dim=1))  # GuidedResampler 不需要Q
            # --- Step2: D-C 结合 ---
            out = self.block(C, attn.mean(dim=1))
            return out
...
class bakThreeCrossAttentionLayerFactory(nn.Module):
    """两步交叉注意力工厂：
       Step1: A-B 对齐 -> D
       Step2: D-C 结合 -> 最终输出
    """
    def __init__(self, args, dim, resolution):
        ...
        if self.is_standard_attention:
            print(f"  -> [工厂] 创建 STANDARD 双阶段交叉注意力层 @ {resolution}")
            # 第一步：A-B
            self.block_ad = StandardUnifiedAttention(dim=dim, num_heads=args.num_heads)
            # 第二步：D-C
            #self.block_bd = self.block_ad
            self.block_efc = StandardUnifiedAttention(dim=dim, num_heads=args.num_heads)
        else:
            print(f"  -> [工厂] 创建 GUIDED SPARCE 双阶段交叉注意力层 @ {resolution}")
            anchor_resolution = args.swin_res_threshold
    ...
    def adaptive_forward(self, m, id):
        B, C, H, W = m.shape
        m_seq = m.flatten(2).transpose(1, 2)  # (B, HW, C)
        id_seq = id.flatten(2).transpose(1,2)
        out_seq ,_ = self.block_ad(m_seq, id_seq, id_seq)
    ...
    def forward(self, A, B, C, D, attn=None):
        ...
        if not self.is_standard_attention:
            ...
            E_seq, _ = self.block_ad(A_seq, D_seq, D_seq)
            F_seq, _ = self.block_ad(B_seq, D_seq, D_seq)
            out_seq, attn_map = self.block_efc(E_seq, F_seq, C_seq)
            out = out_seq.transpose(1, 2).view(B_, C_, H, W)
            return out, attn_map
...
class CanonicalModule(nn.Module):
    """
    第一步: C-B 对齐 -> D (Canonical Feature)
    从源外观(C)和源运动(B)中提炼出不变的、可驱动的身份特征D。
    """
    def __init__(self, args, dim, resolution):
        super().__init__()
        self.is_standard_attention = resolution[0] < args.swin_res_threshold
        ...
        self.blocks = nn.ModuleList()
        if self.is_standard_attention:
            print(f"  -> [模块1] 创建 STANDARD Canonical 注意力层 @ {resolution}")
            self.blocks.append(
                UnifiedTransformerBlock(**common_kwargs)
            )
        else:
            print(f"  -> [模块1] 创建 SWIN Canonical 注意力层 @ {resolution}")
            self.blocks.extend([
                UnifiedSwinBlock(window_size=args.window_size, shift_size=0, **common_kwargs),
                UnifiedSwinBlock(window_size=args.window_size, shift_size=args.window_size // 2, **common_kwargs),
            ])
    ...
    def forward(self, B, C):
        """
        输入:
            B: 源帧运动特征 (B, C, H, W) -> Key/Value
            C: 源外观特征 (B, C, H, W) -> Query
        输出:
            D: Canonical (标准) 特征 (B, C, H, W)
        """
        D = C
        for block in self.blocks:
            # Query=C (源外观), Key/Value=B (源运动上下文)
            # 目的是让外观C根据其自身的运动B进行校准，得到标准特征D
            D = block(D, B, B)
        return D
...
class WarpingModule(nn.Module):
    """
    第二步: D-A 融合 -> 最终输出 (Warped Feature)
    使用驱动运动(A)来驱动Canonical特征(D)，生成最终的运动融合特征。
    """
    def __init__(self, args, dim, resolution):
        super().__init__()
        self.is_standard_attention = resolution[0] < args.swin_res_threshold
        ...
        self.blocks = nn.ModuleList()
        if self.is_standard_attention:
            print(f"  -> [模块2] 创建 STANDARD Warping 注意力层 @ {resolution}")
            self.blocks.append(
                UnifiedTransformerBlock(**common_kwargs)
            )
        else:
            print(f"  -> [模块2] 创建 SWIN Warping 注意力层 @ {resolution}")
            self.blocks.extend([
                UnifiedSwinBlock(window_size=args.window_size, shift_size=0, **common_kwargs),
                UnifiedSwinBlock(window_size=args.window_size, shift_size=args.window_size // 2, **common_kwargs),
            ])
    ...
    def forward(self, A, D):
        """
        输入:
            A: 驱动帧运动特征 (B, C, H, W) -> Key/Value
            D: Canonical 特征   (B, C, H, W) -> Query
        输出:
            out: 最终融合后的特征 (B, C, H, W)
        """
        out = D
        for block in self.blocks:
            # Query=D (标准特征), Key/Value=A (驱动运动)
            # 目的是用新的运动A来驱动标准特征D，完成运动迁移
            out = block(out, A, A)
        return out


class TwoStepCrossAttention(nn.Module):
    """
    集成了 Canonical 和 Warping 两个模块的完整两步交叉注意力层。
    这个结构清晰地分离了两个步骤，方便对中间结果D添加Loss。
    """
    def __init__(self, args, dim, resolution):
        super().__init__()
        self.canonical_module = CanonicalModule(args, dim, resolution)
        self.warping_module = WarpingModule(args, dim, resolution)
    ...
    def forward(self, A, B, C):
        ...
        输出:
            final_out: 最终的输出特征
            D_canonical: 中间的Canonical特征，用于计算一致性Loss
        """
        # --- Step1: 生成 Canonical 特征 D ---
        D_canonical = self.canonical_module(B, C)

        # --- Step2: 进行 Warping/Fusion ---
        final_out = self.warping_module(A, D_canonical)

        # 返回最终结果和中间的D，以便在外部计算Loss
        return final_out
    
class OneCrossAttentionLayerFactory(nn.Module):
