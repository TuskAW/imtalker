...
class DataProcessor:
    def __init__(self, opt):
        ...
        try:
            ...
            print("Local wav2vec model not found, downloading from Hub...")
...
class InferenceAgent:
    ...
    def save_video(self, vid_tensor, fps, audio_path=None):
        ...
        if audio_path:
            ...
            cmd = f"ffmpeg -y -i {raw_path} -i {audio_path} -c:v copy -c:a aac {final_path}"
...
try:
    if os.path.exists(cfg.renderer_path) and os.path.exists(cfg.generator_path):
        ...
        print("Error: Checkpoints not found. Interface will open but inference will fail.")
...
def fn_audio_driven(image, audio, crop, seed, nfe, cfg_scale, progress=gr.Progress()):
    if agent is None: raise gr.Error("Models not loaded properly.")
...
with gr.Blocks(title="IMTalker Demo", css="footer {visibility: hidden}") as demo:
    gr.Markdown("# 🗣️ IMTalker: Efficient Audio-driven Talking Face Generation with Implicit Motion Transfer")
